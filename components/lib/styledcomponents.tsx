...
function StandardInput({ children, className, ...divProps }: InputProps) {
  return (
    <input
      // text-sm
      className={`bg-gray-50 border border-gray-300 text-gray-900 text-sm  focus:ring-teal-400 focus:ring-1 focus:border-slate-700 block w-full p-2 outline-none  ${className}`}
      {...divProps}
    >
      {children}
    </input>
